...
async function removeMovie(id, movie) {
    // const account = await db.Account.findOne({ id: id });
    const account = await getAccount(id);
    account.listFavMovies.splice(account.listFavMovies.findIndex(x => x._id === movie._id), 1);
    account.save();
    return {
        ...basicDetails(account)
    }
...
function basicDetails(account) {
    const {id, title, firstName, lastName, email, role, created, updated, isVerified, listFavMovies} = account;
    return {id, title, firstName, lastName, email, role, created, updated, isVerified, listFavMovies};
}
